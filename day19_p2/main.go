...
func buildGeode(queue []TimeState, timeLimit int, bp Blueprint) int {
	...
	for len(queue) > 0 {
		...
		if start.Robots.Clay > 0 && start.Minerals.Obsidian+start.Robots.Obsidian*(timeLimit-t) < (timeLimit-t)*maxR.Obsidian {
		...
		if start.Minerals.Clay+start.Robots.Clay*(timeLimit-t) < (timeLimit-t)*maxR.Clay {
			// Build a clay robot
			next := start
			deltat := 0
			for {
		...
		if start.Minerals.Ore+start.Robots.Ore*(timeLimit-t) < (timeLimit-t)*maxR.Ore {
			next := start
			deltat := 0
			for {
				deltat++
